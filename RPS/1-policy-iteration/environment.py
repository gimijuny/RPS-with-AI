...
WIDTH = 4  # 그리드월드 가로
TRANSITION_PROB = 1
POSSIBLE_ACTIONS = [0, 1, 2]  # 상, 하, 좌, 우
...
class GraphicDisplay(tk.Tk):
    def __init__(self, agent):
        ...
        self.text_reward(0, 3, "R : 1.0")
        self.text_reward(1, 3, "R : 1.0")
        self.text_reward(2, 3, "R : 1.0")
        self.text_reward(3, 0, "R : -1.0")
        self.text_reward(3, 1, "R : -1.0")
    ...
    def _build_canvas(self):
        ...
        canvas.create_image(50, 350, image=self.shapes[1])
        canvas.create_image(150, 350, image=self.shapes[1])
        canvas.create_image(250, 350, image=self.shapes[1])
        canvas.create_image(350, 50, image=self.shapes[2])
        canvas.create_image(350, 150, image=self.shapes[2])
        canvas.create_image(350, 250, image=self.shapes[2])
        canvas.pack()
    ...
    def reset(self):
        if self.is_moving == 0:
            ...
            self.agent.policy_table[0][3] = []
            self.agent.policy_table[1][3] = []
            self.agent.policy_table[2][3] = []
    ...
    def rectangle_move(self, action):
        ...
        if rules[action] == computer and location[0] < HEIGHT - 1:  # 하
            base_action[1] += UNIT
            self.lose += 1
            print("win: ", self.win, "lose: ", self.lose, "승률: ", self.win / (self.win + self.lose) * 100)
        elif rules[computer] == action and location[1] < WIDTH - 1:  # 우
            base_action[0] += UNIT
            self.win += 1
            print("win: ", self.win, "lose: ", self.lose, "승률: ", self.win / (self.win + self.lose) * 100)
        else:
...
class Env:
    def __init__(self):
        ...
        self.reward[0][3] = 1  # (2,2) 좌표 동그라미 위치에 보상 1
        self.reward[1][3] = 1
        self.reward[2][3] = 1
        self.reward[3][0] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[3][1] = -1  # (2,1) 좌표 세모 위치에 보상 -1
        self.reward[3][2] = -1
        self.all_state = []
